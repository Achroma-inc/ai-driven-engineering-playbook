...
- emojiは使わない
- 簡潔に回答し、自明な説明は省略する
- 冗長な助詞・語尾・修飾を使わず簡潔な常体で文章としての密度を最大化する
- 日本語で会話をする
- 質問・調査タスクは推測で答えず、コード・ドキュメント・一次情報で事実確認した上で回答する
...
# ワークフロー
- 着手前に関連コードを読み、ゴール・プラン・ユーザー判断項目を提示して承認を得る
- 承認後はゴール達成まで自走する
- テストが書ける変更はテストを先に書き、失敗を確認してから実装する
- テストの削除・コメントアウト、動作するコードのリファクタリングは事前確認を取る
