...
このプロジェクトは、全員が同じAIエージェント(Claude Code)で開発を進め、
その過程で得た知識を自動でリポジトリに積み上げていく体制で動いている。
このドキュメントは、新しく参加したあなたがその体制に乗るためのガイド。

対象は **このプロジェクトに関わる全員** — 自社の社員も、クライアント側の正社員も、
別の業務委託のメンバーも区別しない。全員が同じエージェントで、同じやり方で開発する。
それによって、誰が書いたかに関係なく知識が1か所に集まり、チーム全体の資産になる。

## なぜこれをやるのか

管理のためのルールではない。**やったエンジニア自身が一番得をする** 仕組みだから採用している。

- **自分が楽になる。** 仕様の癖、APIの罠、過去の設計判断 — 調べ直しや「前にもハマった気がする」が
  消える。エージェントが着手前に過去の知見(自分のも他人のも)を読んでから作業するので、
  同じ落とし穴を二度踏まない。キャッチアップも調査も速くなる。
- **書く負担がない。** ドキュメントを人間が書いて維持する必要がない。エージェントが作業の中で
  差分を書き、あなたはレビューするだけ。「ドキュメント更新が面倒で放置」が起きない。
- **属人性から解放される。** 知識が人ではなくリポジトリに残るので、業務委託の出入りや
  クライアント側の引き継ぎがあっても、文脈が失われない。「あの人しか知らない」が無くなる。
- **AIの出力が上がる。** 蓄積が厚いほどエージェントがプロジェクトの前提を踏まえた提案をする。
  知識を貯めるほど、自分が使うAIが賢くなって返ってくる。
